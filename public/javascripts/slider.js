...
var $fp_gallery         = $('#mainContent');//$('#fp_gallery');
...
//the main imgContainer for the thumbs structure
...
function openGallery($gallery) {
	gallery_idx = $gallery.index();
    //current gets reset
    current = 0;                  
    //wrapper of each content div, where each image is
    //$fp_content_wrapper = $fp_thumbContainer.find('.imgContainer:nth-child('+parseInt(gallery_idx+1)+')');
    //hide all the other gallerie's thumbs wrappers
    //$fp_thumbContainer.find('.imgContainer').not($fp_content_wrapper).hide();
    //and show this one
    $fp_content_wrapper = $('.imgContainer');
    ...
    */
    ...
    $fp_thumbContainer.animate({'height':'360px'},800,function() {
...
$fp_close.bind('click',function() {
    ...
    //the current thumb
    var $current = $fp_thumbScroller//.find('.imgContainer:nth-child('+parseInt(gallery_idx+1)+')')
...
function getFinalValues($image) {
    ...
    var imgwidth = $image[0].naturalWidth;//theImage.width;
    var imgheight = $image[0].naturalHeight;//theImage.height;
...
function checkClosest() {
    ...
    var idx = 0;    
    $imgContainer = $fp_thumbScroller//.find('.imgContainer:nth-child('+parseInt(gallery_idx+1)+')');
    /*$imgContainer*/.find('.content').each(function(i) {
        var $obj = $(this);
        //the offset left of the element
        var obj_left = $obj.offset().left;
        //the center of the element is its offset left plus 
    ...
    var $new_current = $imgContainer.find('.content:nth-child('+parseInt(idx+1)+')');
...
function slideThumb(way) {
	if (way == 1) {
		++current;
		var $next = $fp_thumbScroller//.find('.imgContainer:nth-child('+parseInt(gallery_idx+1)+')')
									 .find('.content:nth-child('+parseInt(current+1)+')');
		if ($next.length > 0)
		...
		--current;
		var $prev = $fp_thumbScroller//.find('.imgContainer:nth-child('+parseInt(gallery_idx+1)+')')
									 .find('.content:nth-child('+parseInt(current+1)+')');
		if ($prev.length > 0)
